...
def test_empty_csv_treated_as_no_data(tmp_path):
    # A run that recorded zero wells can leave a 0-row CSV; resume must not crash.
    zero = tmp_path / "kgs_qc_records.csv"
    zero.write_text("")            # 0-byte
    assert done_well_ids(zero) == set()
    assert list(merge_keyed(zero, [{"well_id": "A"}])["well_id"]) == ["A"]

    import pandas as _pd
    headerless = tmp_path / "kgs_coverage.csv"
    _pd.DataFrame([]).to_csv(headerless, index=False)   # "\n"
    assert list(merge_keyed(headerless, [{"well_id": "B"}])["well_id"]) == ["B"]
