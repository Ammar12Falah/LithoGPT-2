#!/usr/bin/env python3
"""Item J: the ATCE v1.5 ablation, v2 (items K-S amendments applied 2026-07-26).
Arms A (prior-off), B-linear (prior-on: additive rank-1 linear depth embedding,
renamed from "B" per item O), B2 (prior-on: richer depth featurization --
normalized depth + Athy exp decay + multiscale sin/cos, per item O), C (external
baseline: continuous MSE head), D (optional: explicit trend-residual, Phase 7
Norway Athy fit).

v2 amendments vs the exploratory run committed under item K:
  - item S: tokenizer/scaler are v1's authentic joblib artifacts, loaded directly,
    not refit on FORCE data (narrows the "reimplemented from spec" caveat).
  - item O: Arm B renamed B-linear. Depth-scale blocking pre-check logged before
    training. Arm B2 added with richer depth featurization. Both B-linear and B2
    reported regardless of outcome.
  - item N: parameter counts verified from code; see docs/decisions/
    d2_item_n_paramcount_reversal_2026-07-26.md for the item C reversal this
    produced (neither tied nor untied, with or without bias/sinusoidal-position
    variants, equals 4.8M -- the repo's original 5.2M is closer).
  - item P: generation cap raised so gen_len covers the full remainder of each
    held-out well after priming (previously capped at 1000 samples = 152 m,
    covering as little as 7.7% of the remaining depth on the longest wells).
...
OUT = ROOT / "reports/basinshift/atce_ablation_v2"
...
def b2_depth_features(depth_m, dnorm):
    """item O Arm B2 featurization: normalized depth (per-well [0,1], same signal
    B-linear uses) + Athy exp-decay(absolute depth) + 5 log-spaced sin/cos pairs
    (absolute depth). The exp/sin/cos terms need ABSOLUTE depth in metres --
    dnorm alone (per-well relative position) cannot carry Athy's law, which is
    parameterized in real metres. See the depth-scale precheck in main() for why
    this distinction matters."""
    feats = [dnorm.astype(np.float32)]
...
def main():
    t0 = time.time()
    log("=== ATCE v1.5 ablation v2 (Item J amended: K,L,M,N,S,O,P) START ===")
    ...
    log("EXPLORATORY PREDECESSOR RUN NOTE: this v2 script supersedes the run committed "
        "under item K (labeled EXPLORATORY, not sealed, not reported in the paper).")
    ...
    # item S: load v1's authentic tokenizer/scaler artifacts directly rather than
    # refitting on FORCE data. Both load cleanly (confirmed items B/M). This narrows
    # the manuscript caveat from "reimplemented from spec" to "v1's authentic
    # tokenizer and scaler, transformer retrained from the published specification."
    train_feats = np.concatenate([raw[w][1] for w in train_wells], axis=0)
    try:
        scaler = joblib.load(V1_CKPT_DIR / "scaler_pure.joblib")
        kmeans = joblib.load(V1_CKPT_DIR / "kmeans_pure.joblib")
        assert kmeans.cluster_centers_.shape == (K, len(FEATURES)), \
            f"v1 kmeans shape mismatch: {kmeans.cluster_centers_.shape} vs expected ({K},{len(FEATURES)})"
        tokenizer_provenance = "v1_authentic_artifacts_loaded"
        log("item S tokenizer provenance: LOADED v1's authentic kmeans_pure.joblib + "
            "scaler_pure.joblib directly -- NOT refit on FORCE data.")
        ...
        tokenizer_provenance = f"refit_on_force_fallback: {e!r}"
        log(f"item S tokenizer provenance: v1 artifacts FAILED to load ({e!r}) -- "
            f"FELL BACK to refitting fresh on FORCE 80-train-well data.")
    ...
    depth_feat_linear = {w: depth_norm_seqs[w][:, None] for w in well_names}       # [n,1]
    depth_feat_b2 = {w: b2_depth_features(depth_m_seqs[w], depth_norm_seqs[w])     # [n,12]
                     for w in well_names}
    ...
    log("  DEFECT FOUND (different from the anticipated one, reported per the same "
        ...
        "not silently fixed, per item O's instruction. It bears on how any B-linear or D "
        "null/positive result should be read.")

    # ---------------- Arm A: prior-off, discrete, no depth embed ----------------
    log("=== Arm A: prior-off (v1 authentic tokenizer, transformer reimplemented from spec) ===")
    ...
    # ---------------- Arm B-linear: prior-on, rank-1 linear depth embedding ----------------
    log("=== Arm B-linear: prior-on (rank-1 additive linear depth embedding, renamed from Arm B per item O) ===")
    ...
    # ---------------- Arm B2: prior-on, richer depth featurization ----------------
    log("=== Arm B2: prior-on (normalized depth + Athy exp-decay + 5 log-spaced sin/cos pairs, item O) ===")
    log(f"Arm B2 feature list ({B2_DEPTH_FEAT_DIM} dims): [dnorm, exp(-depth_m/{ATHY_LAMBDA_M})] + "
    ...
    final = dict(
        ...
        arm_b2_feature_list=["dnorm", f"exp(-depth_m/{ATHY_LAMBDA_M})"] +
    ...
    out_path = OUT / "atce_ablation_v2_summary_2026-07-26.json"
    out_path.write_text(payload)
    sha = hashlib.sha256(payload.encode()).hexdigest()
    (OUT / "atce_ablation_v2_summary_2026-07-26.sha256").write_text(sha + "\n")

    raw_path = OUT / "atce_ablation_v2_raw_results_2026-07-26.json"
    raw_path.write_text(json.dumps(results, default=str))
    raw_sha = hashlib.sha256(raw_path.read_bytes()).hexdigest()
    (OUT / "atce_ablation_v2_raw_results_2026-07-26.sha256").write_text(raw_sha + "\n")
