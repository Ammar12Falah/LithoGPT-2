...
### TGS well-log cross-basin generative transformer for well logs: VERIFIED
...
- TimeGPT fine-tuned for well-log forecasting and anomaly detection, KFUPM, arXiv 2412.05681 (December 2024). Time-series cross-basin generative transformer for well logs adapted to logs, not a from-scratch log FM.
- GEM 3D, promptable subsurface cross-basin generative transformer for well logs using well-log prompts for property modeling, arXiv 2507.00419.
...
- Whether any open-weights well-log cross-basin generative transformer for well logs already exists on Hugging Face or GitHub. Search HF hub directly before claiming "first open." Until then the claim is "to our knowledge, no open-weights equivalent exists," stated with that hedge.

## 2. Positioning consequences (post-verification)

"First cross-basin generative transformer for well logs for well logs" is confirmed taken by TGS at industrial scale (1.1M wells) and by WLFM at research scale (1,200 wells). Three doors remain open and compound:
...
- Banned: "largest well-log cross-basin generative transformer for well logs," "first well-log cross-basin generative transformer for well logs," any unhedged "first open" prior to the HF hub check in item 1.
